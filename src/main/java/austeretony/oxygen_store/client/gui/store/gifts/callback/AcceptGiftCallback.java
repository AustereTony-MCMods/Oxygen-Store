...
public class AcceptGiftCallback extends AbstractGUICallback {
    ...
    public AcceptGiftCallback(StoreMenuScreen screen, GiftsSection section, int width, int height) {
    ...
    public void init() {
        ...
        this.addElement(new OxygenTextLabel(4, 12, ClientReference.localize("oxygen_store.gui.store.gifts.callback.acceptGift"), EnumBaseGUISetting.TEXT_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
        this.addElement(new OxygenTextLabel(6, 23, ClientReference.localize("oxygen_store.gui.store.gifts.callback.acceptGift.request"), EnumBaseGUISetting.TEXT_SUB_SCALE.get().asFloat(), EnumBaseGUISetting.TEXT_ENABLED_COLOR.get().asInt()));
    ...
    private void confirm() {
        StoreManagerClient.instance().getStoreOperationsManager().acceptGiftSynced(this.section.getCurrentGiftEntry().getWrapped().getId());
        this.close();
    }
