...
public class AcceptGiftContextAction implements OxygenContextMenuAction {

    private final GiftsSection section;

    public AcceptGiftContextAction(GiftsSection section) {
        this.section = section;
    }

    @Override
    public String getLocalizedName(GUIBaseElement currElement) {
        return ClientReference.localize("oxygen_store.gui.store.context.acceptGift");
    }
    ...
    public boolean isValid(GUIBaseElement currElement) {
        ...
        return entry.getWrapped().getType() == EnumGiftType.PENDING
                || entry.getWrapped().getType() == EnumGiftType.RETURNED;
    }

    @Override
    public void execute(GUIBaseElement currElement) {
        this.section.openAcceptGiftCallback();
    }
}
